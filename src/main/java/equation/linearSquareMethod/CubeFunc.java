...
public class CubeFunc extends AFunc implements IFunc {
    ...
    public double[] getCoefficients(double[] x, double[] y) {
        ...
        double sx5 = getSX(x, 5);
        double sx6 = getSX(x, 6);

        double sy1 = getSX(y, 1);
        double sx1y1 = getSXY(x, y, 1);
        double sx2y1 = getSXY(x, y, 2);
        double sx3y1 = getSXY(x, y, 3);

        double[][] matrix = {{x.length, sx1, sx2, sx3, sy1}, {sx1, sx2, sx3, sx4, sx1y1}, {sx2, sx3, sx4, sx5, sx2y1}, {sx3, sx4, sx5, sx6, sx3y1}};
    ...
    public double f(double X, double[] x, double[] y) {
        ...
        func_name = "phi = (" + ar[3] + ")x^3 + (" + ar[2] + ")x^2 + (" + ar[1] + ")x + (" + ar[0] + ")";
        return ar[0] + ar[1] * X + ar[2] * X * X + ar[3] * X * X * X;
    }

//    public static void main(String[] args) {
//        double[] x = {46.2, 39.4, 36, 33.8, 32.2, 31.2, 29.6, 28.6, 27.6, 26.6, 25.6};
//        double[] y = {0.5, 1, 1.5, 2, 2.5, 3, 4, 5, 6, 7, 8};
//        CubeFunc powFunc = new CubeFunc();
...
//        equationGraphic.drawGraphic(3,x,y);
//    }
}
